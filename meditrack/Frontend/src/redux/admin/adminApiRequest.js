...
import { activateAccountFailed, activateAccountStart, activateAccountSuccess, changePasswordFailed, changePasswordStart, changePasswordSuccess, deleteAccountFailed, deleteAccountStart, deleteAccountSuccess, getAllUsersFailed, getAllUsersStart, getAllUsersSuccess, getSingleUserFailed, getSingleUserStart, getSingleUserSuccess } from "./adminSlice"
...
export const deleteAccount = async (accessToken, dispatch, userID) => {
    ...
    try {
        await axios.put(BASE_ADMIN_URL + "/delete-account/" + userID, null, {
            headers: {
                "Authorization": `Bearer ${accessToken}`
            }
        })
